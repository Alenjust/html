! function(e) {
	...
	function x(e) {
		let t, n = [],
			...
			l = "";
		for (; !e.end();) {
			if (t = e.curr(), /[\('"`]/.test(t) && "\\" !== e.curr(-1)) i.length && "(" != t && t === s(i) ? i.pop() : i.push(t),
				l += t;
			else if ("@" == t) r.length || (l = l.trimLeft()), l.length && (r.push(u.text(l)), l = ""), r.push(b(e));
			else if (/[,)]/.test(t)) {
				if (i.length) ")" == t && i.pop(), l += t;
				else if (l.length && (r.length ? l.length && r.push(u.text(l)) : r.push(u.text((a = l).trim().length ? o.number(+a) ?
						+a : a.trim() : a))), n.push(_(r)), [r, l] = [
						[], ""
					], ")" == t) break
			} else l += t;
			e.next()
	...
	function _(e) {
		...
		if ("text" == n.type && "text" == r.type) {
			let e = i(n.value),
				t = s(r.value);
			"string" == typeof n.value && "string" == typeof r.value && o.pair(e) && o.pair_of(e, t) && (n.value = n.value.slice(
				1), r.value = r.value.slice(0, r.value.length - 1))
		}
		return t
	}
	...
	function $(e) {
		...
		const i = [],
			l = [];
		for (i[r] = []; !e.end();)
			if (t = e.curr(), s && o.white_space(t)) e.next();
			else {
				if (s = !1, "\n" != t || o.white_space(e.curr(-1)))
					if ("," != t || l.length) {
						...
						"@" == t ? (n.value.length && (i[r].push(n), n = u.text()), i[r].push(b(e))) : o.white_space(t) && o.white_space(
							e.curr(-1)) || ("(" == t && l.push(t), ")" == t && l.pop(), n.value += t)
	...
	function L(e, t, n) {
		...
		let u = [];
		for (;
			(n >= 0 && e <= t || n < 0 && e > t) && (u.push(e), e += n, !(r++ >= 1e3)););
	...
	function q(e) {
		...
		return e.includes("<svg") || (e = `<svg ${t}>${e}</svg>`), e.includes("xmlns") || (e = e.replace(/<svg([\s>])/,
			`<svg ${t}$1`)), e
	}
	...
	const Y = J("build_range", e => {
			return l(function(e) {
				...
				for (let e = 1; e < t.length - 1; ++e) {
					...
					if ("-" != i || "-" != t[e - 1])
						if ("-" != i)
							if ("-" != s(r)) r.length && n.push(Q("char", r.pop())), r.push(i);
							else {
								r.pop();
								let e = r.pop();
	...
	const oe = {
			...
			heart: () => le(e => {
				return ue(1.2 * (.75 * re(te(e), 3)), 1.1 * (ee(1 * e) * (13 / 18) - ee(2 * e) * (5 / 18) - ee(3 * e) / 18 - ee(
					4 * e) / 18 + .2), 180)
			}),
			bean: () => le(e => {
	...
	function ge(t) {
		...
		for (; !n.end();) {
			let e = n.curr();
			"(" == e ? (i += e, s.push(e)) : ")" == e ? (i += e, s.length && s.pop()) : s.length ? i += e : fe(e) ? (r.push(i),
				i = "", de(n)) : i += e, n.next()
		}
		return i && r.push(i), r
	}
	...
	var je = {
		...
		nth: ({
			count: e,
			grid: t
		}) => (...n) => n.some(n => ze(n) ? we[n](e - 1) : ke(n, e, t.count)),
		row: ({
			x: e,
			grid: t
		}) => (...n) => n.some(n => ze(n) ? we[n](e - 1) : ke(n, e, t.x)),
		col: ({
			y: e,
			grid: t
		}) => (...n) => n.some(n => ze(n) ? we[n](e - 1) : ke(n, e, t.y)),
		even: ({
			count: e
		}) => t => we.even(e - 1),
		odd: ({
	...
	var Ae = Object.getOwnPropertyNames(Math).reduce((e, t) => (e[t] = (() => (...e) => "number" == typeof Math[t] ? Math[
		t] : Math[t].apply(null, e.map(V))), e), {});
	...
	class Ce {
		...
		compose_rule(e, t, n) {
			...
			"content" == s && (/["']|^(none|var|counter|counters|attr)\(/.test(l) || (l = `'${l}'`)), "transition" == s && (
				this.props.has_transition = !0);
			let u = `${s}:${l};`;
			if (u = be(s, u), "clip-path" == s && (u += ";overflow:hidden;"), "width" != s && "height" != s || Ee(n) || (u +=
					`--internal-cell-${s}:${l};`), $e[s]) {
				let t = $e[s](l, {
	...
	customElements.define("css-doodle", class extends HTMLElement {
		...
		connectedCallback() {
			setTimeout(() => {
				...
				try {
					...
					this.grid_size = P(this.getAttribute("grid")), (e = Oe(n, this.grid_size)).grid && (this.grid_size = e.grid),
						this.build_grid(e)
				} catch (e) {
		...
		update(e) {
			...
			t && (t = `@use:${t};`), e || (e = this.innerHTML), this.innerHTML = e, this.grid_size || (this.grid_size = P(
				this.getAttribute("grid")));
			...
			if (n.grid) {
				...
				if (l !== r || u !== s || o !== i) return Object.assign(this.grid_size, n), this.build_grid(Oe(E(t + e, this.extra),
					this.grid_size))
			}
			this.set_style(".style-keyframes", n.styles.keyframes), this.set_style(".style-container", this.style_size() + n
				.styles.host + n.styles.container), this.set_style(".style-cells", n.styles.cells)
		}
		get grid() {
